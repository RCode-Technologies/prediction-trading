...
On 2026-05-27 the agent placed its first real post-observation position: **US x Iran diplomatic meeting by June 7** (market `2354045`), bought YES at avg **0.373**, sized to the **5% NAV cap**. As of 2026-05-29 the live Gamma book is **bid 0.23 / ask 0.25** — a **~-34% loss on the position** (~-1.8% on the fund). The position is the entire book; everything else is cash.
...
6. **No exit on disconfirmation.** The note listed disconfirming signals ("deal via intermediaries," "FM hardens"). The market then priced exactly that (YES 0.37→0.24). v2 has "no auto-SELL," so the agent watched its own thesis break and did nothing. The disconfirming-signals field is theater if nothing reads it.

### What the rest of the system exposes

7. **The learning loop hasn't learned, and structurally can't yet.** [state/scorecard.json](../../state/scorecard.json) and [state/calibration.json](../../state/calibration.json) are all `null` / `resolved_n: 0`. With ≥3 forecasts/cycle (mostly random ε-probes on sports/elections) and ~30 needed per bucket, useful signal is months away. Resolution is the only feedback channel, and it is slow.
...
11. **Operational fragility.** No cycle has committed since 2026-05-27 20:09Z (it is now 05-29). The scheduler went dark ~44h. The heartbeat that is supposed to detect this is itself a single scheduled point of failure.
