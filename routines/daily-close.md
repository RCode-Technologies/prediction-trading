...
0 research. CLOB freshness only + ≤1 Gamma for `recalibrate.sweep` resolution lookup.
...
Per `skills/commit` § Routine-mapped subjects (`daily-close` rows). If reflection edited strategy, keep both artifacts in the same commit, prefer `feat(strategy)` subject. Body: recap paths, reflection result, notifications, open positions.
