...
| -- | ----- | ---- | ------ | ---------- | ------ | ------- | ---- |
| 2026-06-16-clv-by-liquidity-bucket | Segment CLV by book-liquidity bucket — is our primary learning signal valid, or thin-book noise? | failure / surprise | human_application | medium | surfaced | 2026-06-16 | 2 of 6 forecasts due a CLV snap today had untradeable (empty/one-sided) books; the no-thesis `none` slice (n=32) has the worst CLV (−0.020). Our blended CLV may be thin-book microstructure noise, not prediction error. Read-only recap mirror (liquidity×CLV table) can ship first; canonical compute lives in protected-core `recalibrate` → human applies. |
